...
⚠️ **要求の形を変えたので、実機で新着・入力中が来るか確かめること。**
購読そのものは残してあり範囲を減らしただけだが、公式クライアントが
3 つ送るという観察もあった。だめなら `MEMBER_ROWS` を 3 つに戻すだけ。

残り:

- 巻いた先を頼み直す仕組み (4. の「300 人で止まる」と同じところ)。`watch`
  が範囲を受け取り、変えれば再送されるので、スクロール位置から範囲を出す
  部分だけで足りる
...
| メンバー一覧が 100 人で止まる | 1.3 と同じところ。購読は出す分だけに絞ったので、次は**スクロールで頼み直す**側 |
