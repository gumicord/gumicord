...
**開いた瞬間に前回の画面が出ます。** ログインも接続も待ちません。

```
起動 ──▶ SQLite を同期に読む (数 ms) ──▶ 描く
           └─▶ 背景でログイン → Gateway → 届いたら差し替え
```

待たない根拠は実測です。**Gateway の READY まで 672〜1120 ms かかり、`NFR-001` (コールドスタート 500 ms) に入りません。**

⚠️ **キャッシュは暗号化されていません。** 読んだメッセージの本文が平文で `%APPDATA%\gumicord\cache.db` に残ります。暗号化 (`SEC-020`) は仕様上 M2 です。ログアウト時の完全削除 (`SEC-021`) は入っています。

まだ**未読・既読位置がありません**。READY の `read_state` を読んでいないので、サーバ一覧に印が付きません。Markdown も素のまま出ます (C7)。

詳細は [spec/07-roadmap.md](spec/07-roadmap.md)。
