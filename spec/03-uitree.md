...
| `chat.message.reply_ref` | `MessageData` | 返信元の参照表示 (FR-028)。小アイコンと1行文。押すと元メッセージへ移動する |
...
### settings.screen の振る舞い

左に分類 (`settings.nav`)、右に中身 (`settings.page`) の2欄。行は `overlay.menu.item` を使い回すため、見た目と押下の扱いは操作メニューと同じである。狭い画面 (`Panes::One`) では分類と中身を別画面に出す (下の狭幅行を見よ)。

| 振る舞い | 決め |
|---|---|
| 開く | 利用者パネルの歯車。開いている分類は覚え、掘り下げは忘れる |
| 閉じる | 先頭行の「閉じる」と Esc。決めることがないため確認は出さない。行に当たらなかった押下は捨てる (指が行を外しても画面を失わない) |
| 題名欄 | 残る。画面が場所を取るのは題名欄の下だけであり、窓操作は効いたまま |
| 開いている間の押下 | 画面が全部引き受ける。後ろのチャットへは抜けない |
| 分類 | プラグインとテーマの2つ。今は足さない |
| プラグイン欄 | 一覧では名・版・状態 (有効/無効/拒否/未許可/読込失敗)。掘り下げると権限の箇条書き、操作行 (有効化/無効化/再読み込み/許可のやり直し)、宣言があればそのプラグインの頁 |
| テーマ欄 | 入っているテーマの一覧 (名・版)。開いているものに印が付く。選ぶと即座に切り替わり (`04-theme.md` §5.4)、標準に戻す行もある。画像の取得で起きた失敗の文面もここに出る |
| 狭い画面 | 分類と中身を別画面に出す。分類を選ぶと中身へ進み、先頭の「← 設定」で分類へ戻る。開いた直後は分類。閉じる行と Esc はどちらの画面でも効く |

プラグインの頁の中の操作部品は飾りである (`05-plugin-api.md` §7.4)。押しても画面は閉じない。

### 狭い画面と触れる操作 (X1)

幅で隠れた欄は、触れる操作で取り戻す。窓を広げる手段を持たない携帯のための決めだが、机の狭い窓でも同じに動く。

| 振る舞い | 決め |
|---|---|
| 返信の始め方 | 発言を左へはらう。押して選ぶ操作メニューと同じ返信欄が開く |
| 一覧の棚 (`overlay.drawer`) | 画面の左端から右へはらうか、チャット頭の戻る札で開く。サーバーとチャンネルの一覧が入る。選ぶと閉じる。外側の押下と Esc でも閉じる。開いている間は後ろを覆い (`overlay.scrim`) で暗くし、後ろへの押下は届かせない。開閉は 220ms の滑走で動き、開く途中は端から、閉じる途中は棚の中から指に付いてくる。離すと速さと半分で開閉を決める |
| 一員の面 | 一員欄が隠れている幅では、下から出てくる面 (`overlay.sheet`) に同じ並びを出す。面は横いっぱい・高さ7割までで、下に付く。開くのはチャット頭の人形札ボタン。棚と同じく、開いている間は覆いで後ろを暗くし、後ろへの押下は届かせない。開閉は 220ms の滑走で動き、把手の掴みしろを掴むと下へ付いてくる。離すと速さと半分で開閉を決める。操作メニューを開くと一員面は即座に閉じる（滑走路に載る面は一つだけ） |
| 面のはらい | 棚・面の内側から横・下へはらうと面を閉じる。上への払いは中の一覧のスクロールであり何も起こさない。面の外からの払いは外側の押下と同じく面を閉じるだけで、後ろへの返信などは始まらない。操作メニューの面も下への払いで閉じる。確かめの窓は払いで閉じない |
| 操作メニュー | 狭い幅では下から出てくる面で出す (従来どおり)。広い幅では押した点に出す |
| 縦のはらい | 一覧のスクロールであり、横と判定されなければ何も起こさない |
| 触れと押しの区別 | 短く触れて離すのは押しと同じ。動き出したら押しにはならない |
| 長押し | 動かさず保つと右クリックと同じ操作メニューを開く。対象は既存の操作メニューと同じ。開いたら離しても押しにはならない |

棚と面の中身の行は、並んでいる場所の行を使い回す。見た目と押下の扱いは元の欄と同じである。

押下の内外は面の矩形で決める。後ろの欄と行の ID が重なっていても、面の外の押下は後ろへ抜けず、面の覆いの押下として捨てる。面の中の隙間 (行と行の間) の押下も何も起こさず、閉じもしない。
...
| ~~`settings.*` (設定画面)~~ | **決めた**。`settings.screen` / `nav` / `page` の3個。行とボタンは `overlay.menu.item` と `primitive.text` を使い回す |
...
| ~~Markdown の描画結果をどこまでノードにするか~~ | **決着 (C7)**。行の中の飾りは**ノードにしない** — 並べたノードはそれぞれ独立して折り返すので行末が合わなくなる。`Content::Rich` として 1 つのノードに持たせ、段落・見出し・引用・箇条書き・コードブロックのような**縦に積まれるものだけ**をノードにする ([06-renderer.md 6.6](06-renderer.md#66-飾りの混じった文字-fr-021)) |
