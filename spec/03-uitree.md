...
| 題名欄 | 残る。画面が場所を取るのは題名欄の下だけであり、窓操作は効いたまま || 開いている間の押下 | 画面が全部引き受ける。後ろのチャットへは抜けない |
| 分類 | プラグインとテーマの2つ。今は足さない |
| プラグイン欄 | 一覧では名・版・状態 (有効/無効/拒否/未許可/読込失敗)。掘り下げると権限の箇条書き、操作行 (有効化/無効化/許可のやり直し)、宣言があればそのプラグインの頁 |
