...
左に分類 (`settings.nav`)、右に中身 (`settings.page`) の2欄。行は `overlay.menu.item` を使い回すため、見た目と押下の扱いは操作メニューと同じである。

| 振る舞い | 決め |
|---|---|
| 開く | 利用者パネルの歯車。開いている分類は覚え、掘り下げは忘れる |
| 閉じる | 先頭行の「閉じる」、Esc、外側の押下。決めることがないため確認は出さない |
...
| 一覧の棚 (`overlay.drawer`) | 画面の左端から右へはらうか、チャット頭の戻る札で開く。サーバーとチャンネルの一覧が入る。選ぶと閉じる。外側の押下と Esc でも閉じる |
| 一員の面 | 一員欄が隠れている幅では、下から出てくる面 (`overlay.sheet`) に同じ並びを出す。面は横いっぱい・高さ7割までで、下に付く。開くのはチャット頭の人形札ボタン |
