...
| # | 仮説 | 確かめ方 |
|---|---|---|
| **H4** | **そもそも候補リストを開く操作をしていない。** Microsoft IME はスペース 1 回では第一候補に変換するだけで、候補リストは表示しない。リストが出るのはスペース 2 回目または Ctrl+↓ | **スペースを 2 回押して候補リストが出るか確認する** |
| H5 | TSF モードの Microsoft IME が IMM32 の候補ウィンドウを一切出さない | H4 を否定してから調査 |
| H6 | `ImmAssociateContextEx` / IMC の扱いに起因する別の問題 | H4・H5 を否定してから調査 |

> **注意**: 当初 H5 (TSF) を有力と記載したが、根拠は「TSF と IMM32 で候補位置指定の枠組みが違う」という一般論であり、
> **TSF が候補ウィンドウを「どこにも出さない」ことの説明にはなっていない。** TSF が原因なら「変な場所に出る」はずである。
> 「どこにも出ない」という観測は、むしろ H4 の方が自然に説明する。

**H4 の確認が済むまで、本 ADR の選択肢は確定できない。** 工数見積もりが根本から変わるためである。
