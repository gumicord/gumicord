...
| | |
|---|---|
| ステータス | ❌ **廃止** — [ADR-0006](0006-windows-ime-via-winit.md) が置き換えた |
| 起票日 | 2026-08-14 |
| 決定日 | 2026-08-14 |
| 廃止日 | 2026-08-22 |

> ⚠️ **この ADR の結論は誤っていた。** Windows で TSF テキストストアを自前実装する必要はない。
> 正しい決定は [ADR-0006](0006-windows-ime-via-winit.md) にある。
>
> **それでもこの文書を消さない。** 5 つの仮説を潰して 6 つ目に辿り着く過程と、
> **なぜ最後の 1 つを試さずに承認してしまったか**は、次に同じ罠を踏まないための記録である。
...
## 2026-08-22 追記: 候補ウィンドウが出た。**この決定は誤っていた** ❌

> **[ADR-0006](0006-windows-ime-via-winit.md) が置き換えた。** 以下は誤りに気づいた時点の記録である。

M1.1 のテキスト入力層を実装したところ、**`winit` のまま、TSF テキストストアなしで、変換候補ウィンドウが正しく出た。**

| | |
|---|---|
| IME | **Google 日本語入力** (この ADR が原因究明に使ったのと同じもの) |
| OS | Windows 10 Pro 22H2 (同上) |
| winit | 0.30 (同上) |
| 候補ウィンドウ | **出る。** 入力欄を避けた位置に出る |
| インライン変換 | 成立する。予測候補も出る |
| 位置 | 打ちすぎると隠れるが、**公式 Discord も同じ挙動**なので実用に耐えると判断 |
...
| | S2 (候補が出なかった) | M1.1 (出た) |
|---|---|---|
| 矩形 | キャレット位置の**幅 2px** | **入力欄全体** (約 1030×54) |
| 単位 | 物理ピクセル | 論理ピクセル |

S2 のコメントに残っていた最後の疑い —

> 残る疑い: `set_ime_cursor_area` を呼ぶこと自体が候補ウィンドウを消している。winit は `CANDIDATEFORM` を `dwStyle=CFS_EXCLUDE` で設定するため、指定した矩形を避けるよう IME に指示している。

— は**半分当たっていた**。呼ぶこと自体が悪いのではなく、**避けるべき領域として 2px の矩形を渡していたのが悪かった**と考えられる。実際、候補ウィンドウは入力欄を避けた位置に出ている。

### したがって「結論: 原因は最終的に TSF だった」は誤りである

上の[結論](#結論-原因は最終的に-tsf-だった-)は、**TSF ブリッジ経由でも候補ウィンドウは出せる**という事実と両立しない。原因は TSF ではなく、`set_ime_cursor_area` へ渡す矩形だった。

### まだ確かめていないこと

**この 1 例だけで決定を覆さない。** 少なくとも以下が要る。

| # | 確かめること | なぜ |
|---|---|---|
| 1 | ~~**Microsoft IME** で同じか~~ | ✅ 2026-08-22 に確認。出る |
| 2 | **ATOK** で同じか | 検証機に無い。[ADR-0006](0006-windows-ime-via-winit.md) の見直し条件へ引き継いだ |
| 3 | 高 DPI・複数ディスプレイで位置が壊れないか | `PLT-009` |
| 4 | 文節の区切り直し (Shift+←→) と再変換 | インライン変換として成立しているか |
| 5 | Android / iOS で同じ考え方が通じるか | A2 / I2 の見積もりはここに依存する |

### これが確定したときの影響

| | |
|---|---|
| **P2 (TSF テキストストア)** | **XL → 不要**。M1.1 で唯一の XL であり、クリティカルパスそのもの |
| **A2 / I2 (Android / iOS の IME)** | 「自前実装が要る」前提が消えるなら、M1.2 の最大リスクが下がる |
| ADR-0001 (自前レンダラ) | 見直し条件が 1 つ消える |

**確定したら、この ADR を supersede する ADR-0006 を書く。** 決定を書き換えるのではなく、経緯ごと残す。
