...
| 使うもの | 用途 |
|---|---|
| `take_text_input_state()` | IME 側の本文・選択・変換域の取り出し (ポーリングが公式の使い方) |
| `set_text_input_state()` | 自前の編集 (ハードキー等) の IME への反映 |
| `set_ime_editor_info()` | 欄種別 (本文・メール・パスワード) とアクション (送信・完了・次へ) |
| `take_pending_editor_action()` | ソフトキーボードのアクションボタン (送信に使う) |
